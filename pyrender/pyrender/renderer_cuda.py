...
class RendererCUDA(object):
    ...
    def __init__(self, viewport_width, viewport_height, point_size=1.0):
        ...
        self.viewport_size_x = 0
        self.viewport_size_y = 0

        self.mapResource = None
        self.output_array = None
        self.output_surface = None
        self.cuda_device, self.cuda_context = None, None
        self.kernel_cuda_array_to_surface, self.module_cuda_array_to_surface = None, None
        self.kernel_cuda_array_to_nvc_surface, self.module_cuda_array_to_nvc_surface = None, None
        self.kernel_nvc_surface_to_surface, self.module_nvc_surface_to_surface = None, None
        self.g_img_idx = 0

        # load and compile the edge detection kernel
        glFinish()
        self.cuda_device, self.cuda_context = CodecUtil.create_cuda_context()

        self.edge = CodecUtil.edge
        self.cuda_array_to_surface_str = CodecUtil.cuda_array_to_surface_str
        self.cuda_array_to_nvc_surface_str = CodecUtil.cuda_array_to_nvc_surface_str
        self.nvc_surface_to_surface_str = CodecUtil.nvc_surface_to_surface_str
    ...
    def render(self, scene, flags, seg_node_map=None):
        assert False # this func is replaced by render_just_push, render_read_back
    ...
    def render_just_push(self, scene, flags, seg_node_map=None):
        """Render a scene with the given set of flags.
        ...
        rng = nvtx.start_range( message="_update_context", color='red' )
        self._update_context(scene, flags)
        nvtx.end_range(rng)
        rng = nvtx.start_range( message="_forward_pass_just_push", color='red' )
        ...
        #retval = self._forward_pass(scene, flags, seg_node_map=seg_node_map)
        self.flags = flags
        self.seg_node_map = seg_node_map
        self._forward_pass_just_push(scene, flags, seg_node_map=seg_node_map)
        
        nvtx.end_range(rng)
        rng = nvtx.start_range( message="post", color='red' )
        ...
        return None #return retval

    def render_read_back(self, scene):
        
        retval = self._forward_pass_read_back(scene, self.flags, seg_node_map=self.seg_node_map)
        return retval
    
    def render_read_back_CUDA_OpenGL_Interop(self, scene):
        
        retval = self._forward_pass_read_back_CUDA_OpenGL_Interop(scene, self.flags, seg_node_map=self.seg_node_map)
        return retval
    ...
    def read_color_buf(self):
        ...
        print(f"read_color_buf.glReadPixels")
    ...
    def _forward_pass(self, scene, flags, seg_node_map=None):
        assert False # this func is replace by _forward_pass_just_push, _forward_pass_read_back 
    ...
    def _forward_pass_init_renderer_grid(self, flags):
        # Set up viewport for render
        self._configure_forward_pass_viewport(flags)
    ...
    def _forward_pass_just_push(self, scene, flags, seg_node_map=None):
        # Set up viewport for render
        #self._configure_forward_pass_viewport(flags)
        ...
        #print(f"current viewport is {self.viewport_offset_x, self.viewport_offset_y, self.viewport_size_x, self.viewport_size_y}")
        glViewport(self.viewport_offset_x, self.viewport_offset_y, self.viewport_size_x, self.viewport_size_y)
    ...
    def _forward_pass_read_back(self, scene, flags, seg_node_map=None):
        #print(f"before read_pixels")

        # If doing offscreen render, copy result from framebuffer and return
        if flags & RenderFlags.OFFSCREEN:
            return self._read_main_framebuffer(scene, flags, no_return_depth=True)
        else:
            return

    def _forward_pass_read_back_CUDA_OpenGL_Interop(self, scene, flags, seg_node_map=None):
        #print(f"before read_pixels")

        # If doing offscreen render, copy result from framebuffer and return
        if flags & RenderFlags.OFFSCREEN:
            return self._read_main_framebuffer_CUDA_OpenGL_Interop(scene, flags, no_return_depth=True)
        else:
            return        
    ...
    def _get_camera_matrices(self, scene):
        ...
        P = main_camera_node.camera.get_projection_matrix(
            width=self.viewport_width//self.viewport_grid_x, height=self.viewport_height//self.viewport_grid_y
        )
    ...
    def set_viewport_offset(self, viewport_offset_x, viewport_offset_y, 
        viewport_size_x, viewport_size_y, viewport_grid_x, viewport_grid_y ):
        ...
        self.viewport_grid_x = viewport_grid_x
        self.viewport_grid_y = viewport_grid_y
    ###########################################################################
    # Viewport Management
    ###########################################################################
    ...
    def _configure_main_framebuffer(self):
        ...
        if self._main_fb is None:
            ...
            CodecUtil.cuda_check_errors(cuda.cuCtxSetCurrent(self.cuda_context))

            self.shape = (self.viewport_height, self.viewport_width)
            # allocate an array for the output of the kernel
            self.output_array = CodecUtil.create_empty_array(self.shape, np.uint32)
            # and bind a surface to it
            self.output_surface = CodecUtil.create_surface(self.output_array)

            print(f"self.cuda_device, self.cuda_context: {self.cuda_device, self.cuda_context}")
            
            self.kernel_cuda_array_to_surface, self.module_cuda_array_to_surface = CodecUtil.create_kernel_new(self.cuda_array_to_surface_str, "cuda_array_to_surface", self.cuda_device)
            print(f"self.kernel_cuda_array_to_surface: {self.kernel_cuda_array_to_surface}")
            print(f"self.module_cuda_array_to_surface: {self.module_cuda_array_to_surface}")

            self.kernel_cuda_array_to_nvc_surface, self.module_cuda_array_to_nvc_surface = CodecUtil.create_kernel_new(self.cuda_array_to_nvc_surface_str, "cuda_array_to_nvc_surface", self.cuda_device)
            print(f"self.kernel_cuda_array_to_nvc_surface: {self.kernel_cuda_array_to_nvc_surface}")
            print(f"self.module_cuda_array_to_nvc_surface: {self.module_cuda_array_to_nvc_surface}")

            self.kernel_nvc_surface_to_surface, self.module_nvc_surface_to_surface = CodecUtil.create_kernel_new(self.nvc_surface_to_surface_str, "nvc_surface_to_surface", self.cuda_device)
            print(f"self.kernel_nvc_surface_to_surface: {self.kernel_nvc_surface_to_surface}")
            print(f"self.module_nvc_surface_to_surface: {self.module_nvc_surface_to_surface}")

            self.mapResource = CodecUtil.cuda_check_errors(
            cudart.cudaGraphicsGLRegisterImage(
                self._main_cb,
                GL_RENDERBUFFER,
                cudart.cudaGraphicsRegisterFlags.cudaGraphicsRegisterFlagsWriteDiscard,
            ))
            #print(f"self.mapResource: {self.mapResource}")
    ...
    def _read_main_framebuffer(self, scene, flags, no_return_depth=False):
        ...
        return color_im, depth_im, surf_dst

    def _read_main_framebuffer_CUDA_OpenGL_Interop(self, scene, flags, no_return_depth=False):
        width, height = self._main_fb_dims[0], self._main_fb_dims[1]
        #print(f"framebuffer: {width, height}")
        ...
        if True:            
            ...
            CodecUtil.cuda_check_errors(cudart.cudaGraphicsMapResources(1, self.mapResource, None))
            # get the underlying array
            input_array = CodecUtil.cuda_check_errors(
                cudart.cudaGraphicsSubResourceGetMappedArray(self.mapResource, 0, 0)
            )

            # and bind a texture to the mapped array
            input_texture = CodecUtil.create_texture(input_array, np.uint8)

            gpu_id = 0
            surf_dsts = []
            for gridY in range(self.viewport_grid_y):
                for gridX in range(self.viewport_grid_x):
                    surf_dst = nvc.Surface.Make(nvc.PixelFormat.RGB_PLANAR, width//self.viewport_grid_x, height//self.viewport_grid_y, gpu_id)
                    assert not surf_dst.Empty()
                    dst_plane0 = surf_dst.PlanePtr(0)
                    
                    assert (height//self.viewport_grid_y*3)==dst_plane0.Height()
                    CodecUtil.cuda_check_errors(
                        cuda.cuLaunchKernel(
                            self.kernel_cuda_array_to_nvc_surface,
                            int(np.ceil(width / self.viewport_grid_x / 32)),
                            int(np.ceil(height / self.viewport_grid_y / 32)),
                            1,
                            32,
                            32,
                            1,
            ...
            CodecUtil.cuda_check_errors(cudart.cudaGraphicsUnmapResources(1, self.mapResource, None))

            color_im = np.zeros((1,1))
            depth_im = None
        
        return color_im, depth_im, surf_dsts
