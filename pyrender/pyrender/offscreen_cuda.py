...
class OffscreenRendererCUDA(object):
    ...
    def render_init_grid_renderer(self, flags=RenderFlags.NONE):
        if self._platform.supports_framebuffers():
            flags |= RenderFlags.OFFSCREEN
        self.flags = flags
        self._renderer._forward_pass_init_renderer_grid(self.flags)

    def render_just_push(self, scene, flags=RenderFlags.NONE, seg_node_map=None):
        """Render a scene with the given set of flags.
        ...
        if self._platform.supports_framebuffers():
            #flags |= RenderFlags.OFFSCREEN
            #retval = self._renderer.render(scene, flags, seg_node_map)
            #print(f"cal _renderer.render_just_push")
            self._renderer.render_just_push(scene, self.flags, seg_node_map)
        else:
            assert False
    ...
    def render_read_back(self, scene):
        ...
        if self._platform.supports_framebuffers():
            retval = self._renderer.render_read_back(scene)
        else:
            assert False
    ...
    def render_read_back_CUDA_OpenGL_Interop(self, scene):
        ...
        if self._platform.supports_framebuffers():
            retval = self._renderer.render_read_back_CUDA_OpenGL_Interop(scene)
        else:
            assert False
    ...
    def _create(self):
        ...
        self._platform.make_current()
        self._renderer = RendererCUDA(self.viewport_width, self.viewport_height)
